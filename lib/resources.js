...
module.exports = function (settings, routes, tags) {
  ...
  const paths = _.reduce(routesByPath, (pathsMemo, routes, path) => {
    const operations = _.reduce(routes, (operationsMemo, route) => {
      ...
      if (routesPluginOptions.params) { routesPluginOptions.params = Joi.compile(routesPluginOptions.params).optional() }
      if (routesPluginOptions.query) { routesPluginOptions.query = Joi.compile(routesPluginOptions.query).optional() }
